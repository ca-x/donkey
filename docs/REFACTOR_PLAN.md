...
- 定时同步任务已记录规则 lineage；创建任务、推进计划时间和完成写回均使用明确事务与 fencing。
...
- 镜像拉取历史已支持默认开启、运行时开关、查询和安全清空；保留策略与分页待完善。
...
- 调度流水线固定为：候选过滤 → 冷却节点排除 → 并发容量检查 → 策略排序 → 节点租约 → 失败反馈 → EWMA 更新；
- 策略排序采用响应时间/吞吐加权容量模型；节点并发上限是硬限制，令牌桶仅用于突发与速率背压；
- 记录固定窗口的 RTT、首字节延迟、吞吐、成功率、429/5xx 与 in-flight 样本，使用 EMA 抑制瞬时抖动；
- 可选使用加权 Rendezvous affinity 作为 Blob 到节点的稳定偏好，但不得绕过健康、容量和实时速度判断；
- 节点达到并发上限时等待或选择其他节点，不误报 all nodes failed；
- balanced 保留探索流量，speed-first 优先容量；
- 只使用实际网络表现，不引入地域、IP、ASN 或地理定位；
- 不引入多臂老虎机、P2P 块发现或稀疏块优先级等超出当前规模的算法；
- 参考响应时间加权实现时，必须限制单分片重试次数、保持分片队列游标单调、复用 HTTP client，并避免把完整 Blob 聚合到内存；
- 令牌桶只作为节点级背压，不替代节点健康判断和失败切换；Range end/start 语义统一为闭区间并由测试固定。
- 性能目标只通过实测指标评估，不承诺固定倍数提升；监控至少记录 RTT、首字节延迟、吞吐、成功率、in-flight、429/5xx、切换和重试流量。
- 增加确定性调度和冷却退避测试。
...
- 默认记录通过 Donkey 请求的镜像 tag、Registry 命名空间、解析 digest、状态和时间；
- 不记录 Authorization、Cookie、客户端密码、Registry secret 或访问令牌；
- 设置页面提供即时生效的开关，已有历史不会因关闭开关而删除；
- 提供独立的桌面与移动页面、分页查询和自定义清空确认；
- 增加可配置保留期限或最大记录数，避免长期运行造成数据库无界增长；
- 拉取日志写入失败不得中断 Registry 数据面。
