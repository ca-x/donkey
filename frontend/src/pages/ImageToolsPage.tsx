...
function CopyPanel({ nodes, credentials, openCredentials }: { nodes: NodeView[]; credentials: RegistryCredential[]; openCredentials: () => void }) {
  ...
  const form = useForm<ImageJobInput>({ mode: 'controlled', initialValues: { ...baseJob, kind: 'copy', destination_ref: '' } })
...
function SchedulesPanel({ rules, nodes, credentials }: { rules: ImageSyncRule[]; nodes: NodeView[]; credentials: RegistryCredential[] }) {
  ...
  const form = useForm({ mode: 'controlled', initialValues: { name: '', enabled: true, source_ref: '', source_node_id: null as string | null, source_credential_id: null as string | null, destination_ref: '', destination_credential_id: '', platform_os: 'linux', platform_arch: 'amd64', cron: '0 0 */6 * * *', timezone: 'UTC' } })
...
function TasksPanel({ jobs, canWrite }: { jobs: ImageJob[]; canWrite: boolean }) {
  ...
  const purge = useMutation({ mutationFn: api.purgeImageJob, onSuccess: () => void client.invalidateQueries({ queryKey: ['image-jobs'] }), onError: notifyError(t) })
  ...
  return <Stack gap="sm">{jobs.map((job) => <JobCard key={job.id} job={job} cancel={canWrite && ['pending', 'running'].includes(job.status) ? () => cancel.mutate(job.id) : undefined} purge={canWrite && ['completed', 'failed', 'cancelled', 'skipped'].includes(job.status) ? () => purge.mutate(job.id) : undefined} retry={canWrite ? () => retry.mutate(job.id) : undefined} canceling={cancel.isPending && cancel.variables === job.id} purging={purge.isPending && purge.variables === job.id} retrying={retry.isPending && retry.variables === job.id} />)}</Stack>
}

function JobCard({ job, cancel, purge, retry, canceling, purging, retrying }: { job: ImageJob; cancel?: () => void; purge?: () => void; retry?: () => void; canceling: boolean; purging: boolean; retrying: boolean }) {
  const { t } = useTranslation()
  ...
  return <Paper className="panel image-job-card" data-status={job.status}><Group justify="space-between" align="flex-start" wrap="nowrap"><Box className="mapping-copy"><Group gap="xs"><Text size="xs" fw={680} c={color}>{t(`imageTools.status.${job.status}`, { defaultValue: job.status })}</Text><Text size="xs" c="dimmed">{t(`imageTools.kind.${job.kind}`, { defaultValue: job.kind })}</Text><Text fw={680} truncate>{job.source_ref}</Text></Group>{job.destination_ref && <Text size="sm" c="dimmed" mt={5}>→ {job.destination_ref}</Text>}<Text size="xs" c="dimmed" mt={5}>{t('imageTools.stage')}: {t(`imageTools.stageName.${job.stage}`, { defaultValue: job.stage })} · {job.resolved_digest ?? '—'}</Text></Box><Group gap={3}>{job.status === 'completed' && job.kind !== 'extract' && job.artifact_name && <Tooltip label={t('imageTools.download')}><ActionIcon component="a" href={adminUrl(`/api/image-tools/jobs/${job.id}/artifact`)} aria-label={`${t('imageTools.download')} ${job.source_ref}`}><IconDownload size={17} /></ActionIcon></Tooltip>}{job.status === 'failed' && retry && <Tooltip label={t('imageTools.retry')}><ActionIcon aria-label={`${t('imageTools.retry')} ${job.source_ref}`} loading={retrying} onClick={retry}><IconRefresh size={17} /></ActionIcon></Tooltip>}{cancel && <Tooltip label={t('imageTools.cancel')}><ActionIcon color="red" variant="subtle" aria-label={`${t(job.status === 'completed' ? 'common.delete' : 'imageTools.cancel')} ${job.source_ref}`} loading={canceling} onClick={cancel}><IconX size={17} /></ActionIcon></Tooltip>}{purge && <Tooltip label={t('common.delete')}><ActionIcon color="red" variant="subtle" aria-label={`${t('common.delete')} ${job.source_ref}`} loading={purging} onClick={purge}><IconTrash size={17} /></ActionIcon></Tooltip>}</Group></Group>{job.status === 'running' && <Progress value={progress} mt="md" animated aria-label={`${progress.toFixed(0)}%`} />} {job.error && <Text size="sm" c="red.4" mt="sm">{job.error}</Text>}</Paper>
}
