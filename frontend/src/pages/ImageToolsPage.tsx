...
export function ImageToolsPage() {
  ...
  const jobs = useQuery({ queryKey: ['image-jobs'], queryFn: api.imageJobs, refetchInterval: 2_000 })
...
function SchedulesPanel({ rules, nodes, credentials }: { rules: ImageSyncRule[]; nodes: NodeView[]; credentials: RegistryCredential[] }) {
  ...
  const remove = useMutation({ mutationFn: api.deleteImageRule, onSuccess: () => void client.invalidateQueries({ queryKey: ['image-rules'] }), onError: notifyError(t) })
  return <Stack gap="md"><Paper className="panel"><form onSubmit={form.onSubmit((value) => create.mutate(value))}><Stack><SimpleGrid cols={{ base: 1, sm: 2 }}><TextInput required label={t('imageTools.ruleName')} {...form.getInputProps('name')} /><TextInput required label={t('imageTools.source')} {...form.getInputProps('source_ref')} /><Select label={t('imageTools.pullVia')} description={t('imageTools.sourceRouteHint')} data={sourceNodeOptions(nodes, t('imageTools.direct'))} value={form.values.source_node_id ?? ''} onChange={(value) => { form.setFieldValue('source_node_id', value || null); if (value) form.setFieldValue('source_credential_id', null) }} /><Select label={t('imageTools.sourceCredential')} data={credentialOptions(credentials, t('imageTools.sourceAuthNone'))} value={form.values.source_credential_id ?? ''} onChange={(value) => { form.setFieldValue('source_credential_id', value || null); if (value) form.setFieldValue('source_node_id', null) }} /><TextInput required label={t('imageTools.destination')} {...form.getInputProps('destination_ref')} /><Select required label={t('imageTools.destinationCredential')} data={credentialOptions(credentials, t('imageTools.credentialRequired')).slice(1)} {...form.getInputProps('destination_credential_id')} /><TextInput required label={t('imageTools.cron')} {...form.getInputProps('cron')} /><TextInput required label={t('imageTools.timezone')} {...form.getInputProps('timezone')} /><Switch label={t('common.enabled')} {...form.getInputProps('enabled', { type: 'checkbox' })} /></SimpleGrid><Group justify="flex-end"><Button type="submit" disabled={!form.values.destination_credential_id}>{t('imageTools.createRule')}</Button></Group></Stack></form></Paper>{rules.length === 0 ? <EmptyState title={t('imageTools.noRules')} description={t('imageTools.cron')} /> : rules.map((rule) => <Paper key={rule.id} className="panel"><Group justify="space-between" wrap="nowrap"><Box><Group gap="xs"><Text fw={680}>{rule.name}</Text><Text size="xs" fw={620} c={rule.enabled ? 'green.5' : 'dimmed'}>{rule.enabled ? t('common.enabled') : t('common.disabled')}</Text></Group><Text size="sm" c="dimmed" mt={5}>{rule.source_ref} → {rule.destination_ref}</Text><Text size="xs" c="dimmed" mt={4}>{rule.cron} · {rule.timezone} · {t('imageTools.nextRun')}: {rule.next_run_at ?? '—'}</Text></Box><Group gap={4}><Tooltip label={t('imageTools.runNow')}><ActionIcon aria-label={`${t('imageTools.runNow')} ${rule.name}`} onClick={() => run.mutate(rule.id)}><IconPlayerPlay size={17} /></ActionIcon></Tooltip><Tooltip label={t('common.delete')}><ActionIcon color="red" variant="subtle" aria-label={`${t('common.delete')} ${rule.name}`} onClick={() => remove.mutate(rule.id)}><IconTrash size={17} /></ActionIcon></Tooltip></Group></Group></Paper>)}</Stack>
}
...
function TasksPanel({ jobs, canWrite }: { jobs: ImageJob[]; canWrite: boolean }) {
  ...
  return <Stack gap="sm">{jobs.map((job) => <JobCard key={job.id} job={job} cancel={canWrite ? () => cancel.mutate(job.id) : undefined} retry={canWrite ? () => retry.mutate(job.id) : undefined} />)}</Stack>
}

function JobCard({ job, cancel, retry }: { job: ImageJob; cancel?: () => void; retry?: () => void }) {
  const { t } = useTranslation()
