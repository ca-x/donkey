  ...
  return <Stack gap={24}>
    ...
    <Paper className="panel convert-panel">
      ...
      <Group align="flex-end" mt="md" gap="sm" wrap="nowrap" className="convert-form">
        <TextInput className="convert-input" label={t('domain.original')} value={url} onChange={(event) => setUrl(event.currentTarget.value)} />
...
function MappingDialog({ opened, value, close }: { opened: boolean; value: Mapping | null; close: () => void }) {
  ...
  const editing = value
  const form = useForm<MappingInput>({ mode: 'controlled', initialValues: editing ? { source_host: editing.source_host, upstream_base: editing.upstream_base, public_base: editing.public_base, enabled: editing.enabled } : { source_host: '', upstream_base: '', public_base: '', enabled: true } })
  ...
  return <Modal.Stack><Modal stackId="mapping-editor" opened={opened} onClose={close} title={t(editing ? 'common.edit' : 'domain.add')} centered classNames={{ content: 'polished-modal', overlay: 'polished-overlay' }} transitionProps={{ transition: 'pop', duration: 220, timingFunction: 'cubic-bezier(0.23, 1, 0.32, 1)' }}><form onSubmit={form.onSubmit((input) => save.mutate(input))}><Stack><TextInput required label={t('domain.sourceHost')} {...form.getInputProps('source_host')} /><TextInput required label={t('domain.upstreamBase')} {...form.getInputProps('upstream_base')} /><TextInput required label={t('domain.publicBase')} {...form.getInputProps('public_base')} /><Switch label={t('common.enabled')} {...form.getInputProps('enabled', { type: 'checkbox' })} /><Group justify="space-between" mt="sm">{editing ? <Tooltip label={t('common.delete')}><ActionIcon color="red" variant="subtle" aria-label={`${t('common.delete')} ${editing.source_host}`} onClick={() => setConfirmDelete(true)}><IconTrash size={18} /></ActionIcon></Tooltip> : <span />}<Group><Button variant="default" onClick={close}>{t('common.cancel')}</Button><Button type="submit" loading={save.isPending}>{t('common.save')}</Button></Group></Group></Stack></form></Modal>{editing && <NamedConfirmDialog stackId="mapping-delete-confirm" opened={confirmDelete} title={t('domain.confirmDeleteTitle')} name={editing.source_host} consequence={t('domain.confirmDeleteMessage')} loading={remove.isPending} onCancel={() => setConfirmDelete(false)} onConfirm={() => remove.mutate()} />}</Modal.Stack>
}
