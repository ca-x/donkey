...
function RuntimeSettingsEditor({ config }: { config: import('../types').RuntimeConfig }) {
  ...
  const downloadExport = () => { if (!pendingExport) return; const payload = { ...pendingExport, settings: exportScope.runtime ? pendingExport.settings : ({}) as typeof pendingExport.settings, registry_routes: exportScope.routes ? pendingExport.registry_routes : [], nodes: exportScope.nodes ? pendingExport.nodes : [] }; const url = URL.createObjectURL(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'donkey-config.json'; anchor.click(); URL.revokeObjectURL(url); setPendingExport(null) }
