import { Box, Button, Collapse, FileInput, Group, Modal, NumberInput, Paper, PasswordInput, Select, SimpleGrid, Stack, Switch, Text, TextInput, Title } from '@mantine/core'
...
function AccountSettings() {
  ...
  const save = useMutation({ mutationFn: () => api.updateProfile({ display_name: form.values.display_name, username: user.legacy ? form.values.username : undefined, current_password: form.values.current_password || undefined, new_password: form.values.new_password || undefined }), onSuccess: (updated) => { client.setQueryData(['auth-me'], updated); form.setFieldValue('current_password', ''); form.setFieldValue('new_password', ''); notifications.show({ color: 'green', message: t('settings.profileSaved') }) } })
  return <SettingsPanel icon={<IconSettings size={19} />} title={t('settings.profileTitle')}><SimpleGrid cols={{ base: 1, md: 2 }}><TextInput label={t('settings.displayName')} required {...form.getInputProps('display_name')} />{user.legacy && <TextInput label={t('settings.loginName')} required {...form.getInputProps('username')} />}</SimpleGrid>{user.legacy && <SimpleGrid cols={{ base: 1, md: 2 }}><PasswordInput label={t('settings.currentPassword')} {...form.getInputProps('current_password')} /><PasswordInput label={t('settings.newPassword')} description={t('settings.passwordHint')} {...form.getInputProps('new_password')} /></SimpleGrid>}<Group justify="flex-end"><Button onClick={() => save.mutate()} loading={save.isPending}>{t('common.save')}</Button></Group></SettingsPanel>
}
...
function RuntimeSettingsEditor({ config }: { config: import('../types').RuntimeConfig }) {
  ...
  const exportSettings = useMutation({ mutationFn: api.exportRuntime, onSuccess: (payload) => { const url = URL.createObjectURL(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'donkey-runtime-settings.json'; anchor.click(); URL.revokeObjectURL(url) } })
